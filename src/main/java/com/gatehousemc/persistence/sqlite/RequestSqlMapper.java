...
final class RequestSqlMapper {
    ...
    void savePublication(UUID requestId, com.gatehousemc.domain.PublicationRef publication, Instant now) throws SQLException {
        try (PreparedStatement statement = tx.connection().prepareStatement(
    ...
    List<com.gatehousemc.domain.PublicationRef> publications(UUID requestId) throws SQLException {
        try (PreparedStatement statement = tx.connection().prepareStatement("SELECT provider, external_container_id, external_message_id FROM request_publications WHERE request_id=? AND state='ACTIVE'")) {
            statement.setString(1, requestId.toString());
            try (ResultSet result = statement.executeQuery()) {
                List<com.gatehousemc.domain.PublicationRef> publications = new java.util.ArrayList<>();
                while (result.next()) publications.add(new com.gatehousemc.domain.PublicationRef(result.getString(1), result.getString(2), result.getString(3)));
                return publications;
            }
        }
    }
