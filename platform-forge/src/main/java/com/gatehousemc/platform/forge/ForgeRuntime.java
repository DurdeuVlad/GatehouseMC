...
public final class ForgeRuntime implements AutoCloseable {
    ...
    private ForgeRuntime(MinecraftServer server, GatehouseRuntime delegate) {
        this.server = server;
        this.delegate = delegate;
    }

    static ForgeRuntime start(MinecraftServer server, ModConfig config) throws IOException, SQLException {
        return new ForgeRuntime(server, GatehouseRuntime.start(config, new ForgeVanillaWhitelistAdapter(server)));
    }

    static ForgeRuntime degraded(MinecraftServer server, ModConfig config) {
        return new ForgeRuntime(server, GatehouseRuntime.degraded(config));
    }
