...
public final class GatehouseNeoForgeCommands {
    ...
    private static int unblock(CommandContext<CommandSourceStack> context, Supplier<NeoForgeRuntime> supplier, String reason) {
        return executeText(context, supplier, "unblock " + context.getArgument("username", String.class)
                + (reason == null || reason.isBlank() ? "" : " " + reason));
    }

    private static int reopen(CommandContext<CommandSourceStack> context, Supplier<NeoForgeRuntime> supplier, String reason) {
        try {
    ...
    private static int executeText(CommandContext<CommandSourceStack> context, Supplier<NeoForgeRuntime> supplier, String text) {
    ...
    private static int execute(CommandContext<CommandSourceStack> context, Supplier<NeoForgeRuntime> supplier,
